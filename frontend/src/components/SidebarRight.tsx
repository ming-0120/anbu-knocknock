...
type UserRow = { resident_id: number; name: string; gu: string; address_main?: string; issue?: string; has_task?: boolean; latest_risk_level?: RiskLevel | null; latest_risk_score?: number | null; latest_scored_at?: string | null; scoreText?: string; selectedGu?: string};
...
const SidebarRight = ({ guName, users, onClose, onDetailClick }: SidebarRightProps) => {
  ...
  {guUsers.map((user) => {
    ...
    const issueText = user.issue && user.issue.trim().length > 0
      ? user.issue
      : `위험도: ${scoreStr}`;
      ...
      <div key={user.resident_id} style={cardStyle(user.latest_risk_score)}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "6px" }}>
          ...
          <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
            ...
            <button
              onClick={() => onDetailClick({ ...user, selectedGu: guName })}
              style={smallBtnStyle}
            >
              상세보기
            </button>
